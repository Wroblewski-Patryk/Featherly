...
| P0 | Block style control button semantics | Fill modes and linked-spacing toggles submitted the surrounding editor form and reverted their own pending state. | Shared non-submit controls omitted `type="button"`, so native HTML form semantics treated them as submit actions. | Added explicit button semantics to `FillControl` and `LinkedUnitInput` plus a regression contract; production browser persistence smoke is required after deployment. |
